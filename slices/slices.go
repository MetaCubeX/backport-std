...
import (
	"github.com/metacubex/backport-std/buildin"
...
func Delete[S ~[]E, E any](s S, i, j int) S {
	...
	oldlen := len(s)
	s = append(s[:i], s[j:]...)
	buildin.ClearArray(s[len(s):oldlen]) // zero/nil out the obsolete elements, for GC
	return s
}
...
func DeleteFunc[S ~[]E, E any](s S, del func(E) bool) S {
	...
	buildin.ClearArray(s[i:]) // zero/nil out the obsolete elements, for GC
	return s[:i]
}
...
func Replace[S ~[]E, E any](s S, i, j int, v ...E) S {
	...
	if j == len(s) {
		s2 := append(s[:i], v...)
		if len(s2) < len(s) {
			buildin.ClearArray(s[len(s2):]) // zero/nil out the obsolete elements, for GC
		}
		return s2
	}
	...
	if i+len(v) <= j {
		...
		copy(r[i:], v)
		copy(r[i+len(v):], s[j:])
		buildin.ClearArray(s[tot:]) // zero/nil out the obsolete elements, for GC
		return r
	}
...
func Compact[S ~[]E, E comparable](s S) S {
	...
	for k := 1; k < len(s); k++ {
		if s[k] == s[k-1] {
			...
			buildin.ClearArray(s[k:]) // zero/nil out the obsolete elements, for GC
			return s[:k]
		}
	}
	return s
