...
import (
	...
	. "github.com/metacubex/backport-std/math/rand/v2"
	"math"
	"os"
	"runtime"
	"sync"
...
func nearEqual(a, b, closeEnough, maxError float64) bool {
	...
	return absDiff/buildin.Max(math.Abs(a), math.Abs(b)) < maxError
}
...
func testNormalDistribution(t *testing.T, nsamples int, mean, stddev float64, seed uint64) {
	...
	errorScale := buildin.Max(1.0, stddev) // Error scales with stddev
...
func testExponentialDistribution(t *testing.T, nsamples int, rate float64, seed uint64) {
	...
	errorScale := buildin.Max(1.0, 1/rate) // Error scales with the inverse of the rate
		...
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					...
					errorScale := buildin.Max(1.0, expected.stddev)
