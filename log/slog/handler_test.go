...
func TestDefaultHandle(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			var got string
			var h Handler = newDefaultHandler(func(_ int, b string) error {
				got = string(b)
				return nil
			})
			if test.with != nil {
...
func TestJSONAndTextHandlers(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			wantJSON:  `{"source":{"function":"github.com/metacubex/backport-std/log/slog.TestJSONAndTextHandlers","file":"handler_test.go","line":$LINE},"msg":"message"}`,
		},
		{
